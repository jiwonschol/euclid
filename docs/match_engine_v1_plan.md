...
- **Stage 2 — 포메이션·자율 이동**: 역할별 desired position, 공/점유 기반 팀 형태(공격·수비·전환), first defender/cover/balance, 분리·속도/가속 캡, GK 기본 위치. 게이트: 10명이 공에 몰리지 않음·압박 ≤2명·공격 시 뒤에 2명+GK·폭 유지(물리 lint 이식).
- **Stage 3 — 공 소유·행동**: 드리블·지상/공중 패스·수신·루즈볼·인터셉트·슛·블록·득점. utility AI(§6). 게이트: 패스 순간이동 없음·중앙선 난사 없음·지원 삼각형.
