...
## 0.1 게임 정체성·우선순위 (2026-07-14 디자이너, 최우선)

**이 게임은 축구 시뮬이 아니다.** *"축구라는 보드게임판을 보며 감독(유저)이 컴퓨터의 수에 어떻게
대응하는가"* 를 보는 게임이다. 경기 장면은 물리 재현이 아니라 그 대응이 **읽히는 하이라이트**다.
따라서:
- **가독성 > 시뮬 정밀도.** 패스가 유기적으로 이어지고, 수비가 끌려나오거나(압박) 물러서는(블록)
  반응이 **길고 명확히 보여야** 한다. "한두 번 패스 후 슛/소멸" 반복은 near-bug — 점유가 이어지고
  전개가 쌓여야 한다. (원인: 태클/가로채기 과다 + 조급한 슛 → 튜닝으로 완화, `data/engine.json`.)
- **지시 = 오디블(전달 중 교체).** 카드(지시)를 내면 **'전달 중'**(아직 그라운드에 안 닿음). 그 사이
  김성주 신호("상대 공격적·뒷공간 위험")를 읽고 **전달 중인 지시를 다른 지시로 교체(오디블)** 할 수
  있다 — **덮어쓰기 = 비용 1회만**(공격→역습이 2회 소모 금지). UI는 **활성 지시 / 전달 중 지시**가
  무엇인지 항상 명확히 표시한다. (구 §8-B 개정. 새 엔진 Stage 6 에서 구현, 구 main 은 미수정.)
- **우선순위 재조정:** 보드 가독성(패스 전개·수비 반응) → **지시/오디블 카드 시스템(앞당김)** →
  오프사이드·GK 정밀화 등 순수 시뮬 요소는 가독성에 필요한 만큼만 뒤로.
