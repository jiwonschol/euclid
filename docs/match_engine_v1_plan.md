...
- **중계(텍스트)가 코어다.** 완벽한 영상은 불가능 — FM처럼 **메시지·스탯·그래프가 유저 판단의
  근거**이고 그 근거는 '글'이다. 바둑알은 그 실현·보조일 뿐, 실제 경기는 **유저의 상상**에서 벌어진다.
  따라서 **시스템/중계 메시지를 훨씬 많이, 경기 상태를 실시간 broadcast**한다(흐름·압박·영역·측면·
  역습·이벤트·스탯). 구현 `js/game/commentary.js`(상태 기반·결정론·rng 미소비) + `data/commentary.json`
  (템플릿). 뷰어는 **중계 피드 + 스탯을 1차 표면**, 피치는 보조로 재배치.
- **공격/수비 움직임(2026-07-15 재지시):** "패스→슛"만이면 축구가 아니다. 오프-볼 움직임이 보여야:
  `js/game/attack.js`(러너 침투·서포트 콤비·풀백 오버랩·레이트런) + `defend.js`(마킹으로 침투 봉쇄).
  **오프사이드는 형상 front가 아니라 실제 세컨드-라스트 수비(`offsideLineX`)에 러너를 묶어** 캠핑 제거.
  슛은 박스라고 무조건이 아니라 **GK가 못 막는 빈 곳(openness)**일 때만. 공격 = **측면/중앙 2분법**
  (`tactics.attackZone`, 카드 측면공격/중앙공격): 측면=크로스 전개·원터치 마무리, 중앙=스루 침투.
...
- **Stage 4 — 규칙(오프사이드·재개) ✅**: 오프사이드 스냅샷 판정(상대진영+공앞+세컨드라스트 수비앞)·관여 시 프리킥·AI 오프사이드 회피, 터치라인 스로인/골라인 골킥·코너 분류. **핵심: 박스 상주 억제로 점유 편중 88%→53% 해결, 골 현실화(≈5)**. (반칙/옐로·레드/PK·연장은 프로토타입 범위 밖.)
- **Stage 5 — 골키퍼**: **미구현**(2026-07-15 감사 정정). 현재는 `role==='GK'` 분기 + 확률 선방뿐으로, 계획서 §9 가 명시적으로 금지한 방식이다. 전용 모듈·다이빙 y·박스 손 판정·백패스·1대1 모두 없음. → docs/plan_audit.md
- **Stage 6 — 감독 카드 ✅**: `js/game/directives.js` — CP 자원·손패(지시 7 + 교체 3)·**오디블(전달중 교체=비용 1회)**·상대 AI 전술전환 + **김성주 읽기-대응 루프**. 교체 3종(fw/mf/df) subBoost. 카드는 tactics→shape/AI에 실제 개입(장식 아님). 뷰어 손패 UI.
- **Stage 7 — 연출**: 시네마틱 확대는 **실시간 중계 텍스트로 대체**(§0.1 정체성: 텍스트가 코어, 바둑판은 보조). 별도 컷인 미구현.
- **Stage 8 — 안정성·튜닝 ✅**: `sim/stability.mjs` 100경기 풀스택 예외/NaN/데드락/폭주 0·재현성. README 작성. 밸런스는 data/*.json 지속 튜닝.
