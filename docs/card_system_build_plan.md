...
> ## ⚠️ 2026-07-15 감사 정정 — 아래 S1~S7 체크는 과대 표기다
> `docs/plan_audit.md` 의 코드 직접 감사 결과, 이 문서의 "완료" 표기는 **구현된 범위 안에서만** 사실이었다.
> 계획서 §12 기준으로 다음이 **미구현인 채 S5/S7 완료로 표기**되어 있었다:
> **stale swap**(`staleSwapSec` 데드 설정) · **맥락 카드 1~2장**(4장 전부 주입 + `handMax` 우회 → 손패 10장 가능) ·
> **`scope`**(`applyMod` 가 `m.scope` 를 안 읽음 — ROLE/ZONE 추가 시 조용히 팀 전역 적용) ·
> **CardTiming 3종 데드 분기**(`pendingEncounter`/`restart`/`STOPPAGE` 는 아무도 안 쓰는 필드) · **§13 거츠 전무**.
> 또 커밋 `52562bf` 의 "모든 카드가 실제로 경기를 바꿈"은 **거짓**이었다(`transition`·`overlapSide`·`gutsThresh` 소비자 0).
> **교훈: `sim:cards` 59/59 green 이 이 결함을 하나도 못 잡았다. 린트가 스펙이 아니라 구현을 따라 쓰였기 때문 — 게이트 green ≠ 계획서 충실.**
